import { fontFamily, fontSize } from "@mui/joy/styles/styleFunctionSx";

export const departobj = [
  // {
  //   id: "#agri",
  //   title: "Agricultural Engineering",
  //   img: "/DepartmentAssets/agri.webp",
  //   des: "test",
  //   color: "#67ff45",
  //   bgtext: "Agri",
  // },
 
  // {
  //   id: "#aids",
  //   title: "Artificial\tIntelligence\t& Data\tScience",
  //   img: "/DepartmentAssets/aids.webp",
  //   des: "test",
  //   color: "#ff3b41",
  //   bgtext: "data",
  
  // },

  // {
  //   id: "#aiml",
  //   title: "Artificial\tIntelligence\t& Machine\tLearning",
  //   img: "/DepartmentAssets/ai.webp",
  //   des: "test",
  //   color: "#37c0ff",
  //   bgtext: "Machine",
  // },

  // {
  //   id: "#bme",
  //   title: "Bio\tMedical Engineering",
  //   img: "/DepartmentAssets/bme.webp",
  //   des: "test",
  //   color: "#ff58c2",
  //   bgtext: "BioMed",
  // },
 
  // {
  //   id: "#chem",
  //   title: "Chemical Engineering",
  //   img: "/DepartmentAssets/chem.webp",
  //   des: "test",
  //   color: "#ff6d41",
  //   bgtext: "Chem",
  // },

  // {
  //   id: "#civil",
  //   title: "Civil Engineering ",
  //   img: "/DepartmentAssets/civil.webp",
  //   des: "test",
  //   color: "#ff3b41",
  //   bgtext: "Civil",
  // },
  
  // {
  //   id: "#cse",
  //   title: "Computer\tScience &\tEngineering",
  //   img: "/DepartmentAssets/cse.webp",
  //   des: "test",
  //   color: "#67ff45",
  //   bgtext: "Compu",
  // },
  
  // {
  //   id: "#csiot",
  //   title: "Cyber\tSecurity\t& Internet\tOf\tThings",
  //   img: "/DepartmentAssets/csiot.webp",
  //   des: "test",
  //   color: "#CDCD00",
  //   bgtext: "network",
  // },

  // {
  //   id: "#ece",
  //   title: "Electronics\t& Communication\tEngineering",
  //   img: "/DepartmentAssets/ece.webp",
  //   des: "test",
  //   color: "#ff58c2",
  //   bgtext: "Elect",
  //   mbtm: "-30px",
  // },
  
  // {
  //   id: "#eee",
  //   title: "Electrical\t& Electronics\tEngineering  ",
  //   img: "/DepartmentAssets/eee.webp",
  //   des: "test",
  //   color: "#ff6d41",
  //   bgtext: "Elect",
  //   mbtm: "-30px",
  // },
  
  // {
  //   id: "#eie",
  //   title: "Electronics\t& Instrumentation\tEngineering",
  //   img: "/DepartmentAssets/eie.webp",
  //   des: "test",
  //   color: "#ff3b41",
  //   bgtext: "iInst",
  //   mbtm: "-30px",
  // },
  
  // {
  //   id: "#it",
  //   title: `Information Technology`,
  //   img: "/DepartmentAssets/it.webp",
  //   des: "test",
  //   color: "#37c0ff",
  //   bgtext: "Tech",
  // },

  // {
  //   id: "#med",
  //   title: "Medical Electronics",
  //   img: "/DepartmentAssets/med.webp",
  //   des: "test",
  //   color: "#ff3b41",
  //   bgtext: "Medi",
  // },

  // {
  //   id: "#mech",
  //   title: "Mechanical Engineering",
  //   img: "/DepartmentAssets/mech.webp",
  //   des: "test",
  //   color: "#67ff45",
  //   bgtext: "Mech",
  // },
  
  // {
  //   id: "#mba",
  //   title: "Master\tof\tBusiness Administration",
  //   img: "/DepartmentAssets/mba.webp",
  //   des: "test",
  //   color: "#CDCD00",
  //   bgtext: "Busin",
  // },

  {
    id: "#agri",
    title: "Agricultural Engineering",
    img: "/DepartmentAssets/agrinew.webp",
    des: "test",
    color: "#67ff15",
    bgtext: "Agri",
    mbtm: "-30px",
  }, 

  {
    id: "#bme",
    title: "Bio\tMedical Engineering",
    img: "/DepartmentAssets/bmenew.webp",
    des: "test",
    color: "#FF0000",
    bgtext: "BioMed",
    mbtm: "-30px",
  },
 
  {
    id: "#chem",
    title: "Chemical Engineering",
    img: "/DepartmentAssets/chemnew.webp",
    des: "test",
    color: "#00FFFF",
    bgtext: "Chem",
    mbtm: "-30px",
  },

  {
    id: "#civil",
    title: "Civil Engineering ",
    img: "/DepartmentAssets/civilnew.webp",
    des: "test",
    color: "#D2691E",
    bgtext: "Civil",
    mbtm: "-30px",
  },

  {
    id: "#ece",
    title: "Electronics\t& Communication\tEngineering",
    img: "/DepartmentAssets/ecenew.webp",
    des: "test",
    color: "#FCC200",
    bgtext: "Elect",
    mbtm: "-30px",
  },
  
  {
    id: "#eee",
    title: "Electrical\t& Electronics\tEngineering  ",
    img: "/DepartmentAssets/eeenew.webp",
    des: "test",
    color: "#CC5500",
    bgtext: "Elect",
    mbtm: "-30px",
  },
  
  {
    id: "#eie",
    title: "Electronics\t& Instrumentation\tEngineering",
    img: "/DepartmentAssets/eienew.webp",
    des: "test",
    color: "#FF00FF",
    bgtext: "iInst",
    mbtm: "-30px",
  },

  {
    id: "#mech",
    title: "Mechanical Engineering",
    img: "/DepartmentAssets/mechnew.webp",
    des: "test",
    color: "#D3D3D3",
    bgtext: "Mech",
    mbtm: "-30px",
  },

  {
    id: "#med",
    title: "Medical Electronics",
    img: "/DepartmentAssets/mednew.webp",
    des: "test",
    // color: "#FFD760",
    color: "#37c0ff",
    bgtext: "Medi",
    mbtm: "-30px",
  },

  {
    id: "#scoft",
    // title: "Saveetha\tConsortium\tfor\tFuture Technologies",
    title: "SCOFT",
    img: "/DepartmentAssets/scoftnew.webp",
    des: "test",
    color: "#CDCD00",
    bgtext: "SCOFT",
    mbtm: "-30px",
  },

  {
    id: "#mba",
    title: "Master\tof\tBusiness Administration",
    img: "/DepartmentAssets/mbanew.webp",
    des: "test",
    color: "#008080",
    bgtext: "Busin",  
    mbtm: "-30px",
  },
  
  
];

export const comobj = [

  // {
  //   id: "#paper",
  //   title: "Paper Presentation",
  //   img: "/DepartmentAssets/paper.webp",
  //   des: "test",
  //   color: "#ff0000",
  //   bgtext: "Paper",
  // },

  // {
  //   id: "#project",
  //   title: "Project Display",
  //   img: "/DepartmentAssets/PD.webp",
  //   des: "test",
  //   // color: "#4da6ff",
  //   color: "#D3D3D3",
  //   bgtext: "Proj",
  // },

  {
    id: "#poster",
    title: "Poster Presentation",
    img: "/DepartmentAssets/pp.webp",
    des: "test",
    // color: "#ff58c2",
    color: "#008080",
    bgtext: "Pros",
  },
  {
    id: "#bestoutofwaste",
    title: "Best-out-of-Waste",
    img: "/DepartmentAssets/ce1.webp",
    des: "test",
    // color: "#ffff00",
    color: "#00ff00",
    bgtext: "Pros",
  },
];

export const splevnt= [
  

  {
    id: "#CLUB1",
    title: "TECHNICAL\tMURDER\tMYSTERY",
    img: "/DepartmentAssets/lb.webp",
    des: "test",
    // color: "#ac00e6",
    color: "#F4A261",
    bgtext: "LC",
  },

  {
    id: "#CLUB2",
    title: "VOICE OVER",
    img: "/DepartmentAssets/lb.webp",
    des: "test",
    // color: "#ac00e6",
    color: "#F4A261",
    bgtext: "LC",
  },

  {
      id: "#ipl",
      title: "IPL AUCTION",
      img: "/DepartmentAssets/iplogo.webp",
      des: "test",
      color: "#ffff00",
      bgtext: "IPL",
    },

  {
    id: "#Corporate Walk",
    title: "CORPORATE WALK",
    img: "/DepartmentAssets/cw.webp",
    des: "test",
    color: "#ac00e6",
    bgtext: "COR",
  },

  {
    id: "#ADZAP",
    title: "ADZAP",
    img: "/DepartmentAssets/adzaplogo.webp",
    des: "test",
    color: "#0080ff",
    bgtext: "TENT",
  },

  // {
  //   id: "#DUBSTAR",
  //   title: "DUBSTAR",
  //   img: "/DepartmentAssets/D.webp",
  //   des: "test",
  //   color: "#00ff00",
  //   bgtext: "DUBS",
  // },

  // {
  //   id: "#AUTO EXPO",
  //   title: "AUTO EXPO",
  //   img: "/DepartmentAssets/autoco.webp",
  //   des: "test",
  //   color: "#FF0000",
  //   bgtext: "expo",
  // },

  // {
  //   id: "#STYLE WITH CANVA",
  //   title: "STYLE\tWITH CANVA",
  //   img: "/DepartmentAssets/cvn.webp",
  //   des: "test",
  //   color: "#FFCCE6",
  //   bgtext: "canva",
  // },

  // {
  //   id: "#STUMP SAGA",
  //   title: "STUMP SAGA",
  //   img: "/DepartmentAssets/c.webp",
  //   des: "test",
  //   color: "#4da6ff",
  //   bgtext: "Proj",
  // },

  // {
  //   id: "#ADAPTUNE",
  //   title: "ADAPTUNE",
  //   img: "/DepartmentAssets/dance.webp",
  //   des: "test",
  //   color: "#4da6ff",
  //   bgtext: "Proj",
  // },

];